...
old="""        self.page.get_by_role('button',name='Build a plan',exact=True).click();self.confirm('Build plan')
...
"""
new="""        self.page.get_by_role('button',name='Build a plan',exact=True).click();self.confirm('Build plan')
...
"""
if s.count(old)!=1:raise SystemExit(f'plan wait target count {s.count(old)}')
...
print('Made confirmation-backed browser assertions wait for committed state.')
