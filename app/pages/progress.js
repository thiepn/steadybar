...
export function progressPage() {
    const data = store.snapshot(), page = el('div', { class: 'page' }, pageHeader('EVIDENCE, NOT GUESSWORK', 'Your progress', 'Practice time is useful. Clean, repeatable improvement is the point.'));
    if (!finishedSessions(data.sessions).length) {
        page.append(empty('Your progress starts with a session.', 'Nothing is estimated or invented here. Practice, record an honest attempt, and return to see what changed.', link('Start practice', '/practice', 'button primary', 'play'), 'progress'));
        return { node: page };
    }
    ...
    const recorded = data.exercises.find(e => exerciseAttempts(data.sessions, e.id).length > 0);
    if (recorded)
        exercise.value = recorded.id;
    ...
    const draw = () => {
        ...
        if (range.value === 'custom') {
            ...
            const date = new Date();
            date.setDate(date.getDate() - Number(range.value) + 1);
        ...
        if (start && end && start > end) {
            content.append(el('p', { class: 'form-error', role: 'alert' }, 'The start date must come before the end date.'));
            return;
        }
        ...
        if (!sessions.length) {
            content.append(empty('No sessions in this range.', 'Choose a wider date range or start a session today.', link('Practice', '/practice', 'button secondary', 'play')));
            return;
        }
        const time = el('section', { class: 'panel' }, sectionHeader('Time at the instrument'), dayChart(days));
        const mix = el('section', { class: 'panel' }, sectionHeader('Practice distribution'));
        for (const item of distribution)
            mix.append(el('div', { class: 'distribution-row' }, el('div', { class: 'split' }, el('strong', {}, titleCase(item.category)), el('span', { class: 'muted small' }, `${duration(item.seconds)} · ${item.percent.toFixed(1)}%`)), progressBar(item.percent / 100, `${item.category} practice`)));
        if (!distribution.length)
            mix.append(el('p', { class: 'muted' }, 'These sessions have no recorded active time.'));
        ...
        if (series.length)
            tempo.append(el('details', { class: 'data-details' }, el('summary', {}, 'View tempo data'), el('table', {}, el('thead', {}, el('tr', {}, el('th', { scope: 'col' }, 'Date'), el('th', { scope: 'col' }, 'Best clean BPM'))), el('tbody', {}, series.map(p => el('tr', {}, el('td', {}, p.date), el('td', {}, p.bpm)))))));
        content.append(tempo);
        const first = start || days[0]?.date || localDate(), last = end || localDate();
        const numberOfDays = Math.max(1, Math.round((new Date(`${last}T12:00:00`).getTime() - new Date(`${first}T12:00:00`).getTime()) / 86400000) + 1);
        const consistency = el('section', { class: 'panel' }, sectionHeader('A sustainable rhythm'), el('div', { class: 'stats-strip inset-stats' }, stat('Completed this week', calculateWeeklySessionCount(data.sessions)), stat('Avg. sessions / week', (sessions.filter(s => s.status === 'completed').length / (numberOfDays / 7)).toFixed(1)), stat('Time per active day', duration(days.length ? total / days.length : 0))), el('p', { class: 'field-hint' }, 'Weekly sessions are completed sessions, grouped Monday–Sunday in your local timezone. Time totals also retain practice from sessions ended early.'));
        const goals = el('section', { class: 'panel' }, sectionHeader('Goal progress', 'Current status, independent of chart range'));
        if (!data.goals.length)
            goals.append(el('p', { class: 'muted' }, 'Create a goal to connect these results to your next step.'), link('Create a goal', '/goals', 'text-link', 'arrow'));
        else
            for (const g of data.goals.slice(0, 5)) {
                const p = goalProgress(g, data);
                goals.append(el('div', { class: 'goal-summary' }, el('strong', {}, g.title), el('p', { class: 'small muted' }, p.label), progressBar(p.fraction, g.title)));
            }
    ...
    exercise.addEventListener('change', draw);
    draw();
    page.append(el('div', { class: 'range-toolbar' }, field('Date range', range), custom), content);
    return { node: page };
}
