...
export function settingsPage() {
    ...
    const appearance = el('section', { class: 'panel' }, sectionHeader('Appearance'), el('div', { class: 'theme-options' }, ['system', 'light', 'dark'].map(theme => { const b = button(theme[0].toUpperCase() + theme.slice(1), async () => { if (dirty && !await confirmAction('Discard unsaved preferences?', 'Save your practice defaults first, or discard those edits to change appearance.', 'Discard edits'))
        return; dirty = false; await store.settings({ theme }); notify('Appearance saved.'); }, `theme-choice ${settings.theme === theme ? 'selected' : ''}`, theme === 'dark' ? 'moon' : theme === 'light' ? 'sun' : 'settings'); b.setAttribute('aria-pressed', String(settings.theme === theme)); return b; })), el('p', { class: 'field-hint' }, 'System follows your device’s appearance. No remote fonts or theme assets are required.'));
    ...
    prefs.addEventListener('input', () => { dirty = true; });
    prefs.addEventListener('change', () => { dirty = true; });
    ...
    file.addEventListener('change', async () => {
        ...
        try {
            if (selected.size > 100 * 1024 * 1024)
                throw new Error('This backup is larger than the 100 MB import limit. No data was changed.');
            ...
            if (!await confirmAction('Replace this device’s practice data?', `Validated backup from ${formatDate(backup.exportedAt, true)}: ${counts}. This replaces all current data, including settings. A safety backup of your current data will be downloaded first.`, 'Back up & replace', true))
                return;
            if (practice.session?.status === 'active' && !practice.external)
                await practice.pause();
            audio.stop();
            await withWorkspaceIdle(async () => { await exportBackup(); await restoreBackup(backup); });
            location.reload();
        }
        catch (e) {
            notify(e instanceof Error ? e.message : 'The backup could not be restored. Existing data was not changed.', 'error');
        }
        finally {
            file.value = '';
        }
    ...
    if (navigator.storage?.estimate)
        void navigator.storage.estimate().then(estimate => { storage.textContent = `Using approximately ${((estimate.usage || 0) / 1024 / 1024).toFixed(1)} MB${estimate.quota ? ` of ${(estimate.quota / 1024 / 1024 / 1024).toFixed(1)} GB available browser quota` : ''}. This is browser-origin storage, not cloud storage.`; }).catch(() => { storage.textContent = 'Storage quota information is not available in this browser.'; });
    else
        storage.textContent = 'Storage quota information is not available in this browser.';
    const persist = button('Protect local storage', async () => {
        if (!navigator.storage?.persist) {
            notify('This browser does not expose persistent-storage requests. Keep regular backups.', 'info');
            return;
        }
        const granted = await navigator.storage.persist();
        notify(granted ? 'Persistent storage is enabled. Keep exporting backups as well.' : 'The browser did not grant persistent storage. Your data is still saved, but backups are important.', 'info');
    }, 'secondary');
    ...
    const refreshOffline = () => {
        ...
        offlineError.textContent = pwaState.error || '';
        offlineError.hidden = !pwaState.error;
    };
    refreshOffline();
    window.addEventListener('pwa-state', refreshOffline);
    ...
    const reset = () => formDialog('Reset Steadybar', [
        ...
        if (formText(form, 'confirm') !== 'RESET')
            throw new Error('Type RESET exactly to confirm.');
        if (practice.session?.status === 'active' && !practice.external)
            await practice.pause();
        audio.stop();
        await withWorkspaceIdle(async () => { await exportBackup(); await replaceData(seedData()); });
        location.reload();
    }, 'Back up & reset');
