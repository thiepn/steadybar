...
function applyTheme() { const theme = store.snapshot().settings.theme; document.documentElement.dataset.theme = theme; document.querySelector('meta[name="theme-color"]')?.setAttribute('content', theme === 'dark' ? '#171d19' : '#f5f5f0'); }
function route(path) {
...
function render() {
    ...
    if (active)
        ...
        for (const [href, label, symbol] of navigation) {
            ...
            if (href === '/library' || href === '/goals' || href === '/settings')
        ...
        const search = button('Quick find', openSearch, 'search-trigger', 'search');
        ...
        for (const [href, label, symbol] of [navigation[0], navigation[1], navigation[3], navigation[8]]) {
            const a = el('a', { href: `#${href}`, class: activeLink(href, path) ? 'active' : '' }, icon(symbol, 21), el('span', {}, label));
            if (activeLink(href, path))
                a.setAttribute('aria-current', 'page');
            mobile.append(a);
        }
