...
export function practicePage() {
    ...
    const free = el('section', { class: 'panel' }, sectionHeader('Free practice'), el('p', { class: 'muted' }, 'Practice with a timer and metronome.'), el('div', { class: 'form-grid' }, field('Minutes', minutes), field('BPM', bpm)), button('Start free practice', async () => { if (minutes.reportValidity() && bpm.reportValidity())
    ...
    if (data.settings.instrument === 'Drums')
        page.append(el('section', { class: 'panel' }, sectionHeader('Drum exercises'), el('div', { class: 'quick-exercises' }, recentExercises.map(e => button(e.name, () => launchPractice([exerciseBlock(e)]), 'quick-exercise', 'play')))));
    return { node: page };
}
export function activePracticePage() {
    ...
    leave.setAttribute('aria-label', 'Save & leave');
    leave.title = 'Pause, save, and leave practice';
    const header = el('header', { class: 'practice-header' }, leave, el('div', { class: 'actions' }, focusButton, button('Finish session', async () => { if (await confirmAction('Finish this session?', 'Your time, attempts, and notes will be saved. Unfinished future blocks will be marked skipped.', 'Finish session')) {
    ...
    const transition = el('div', { class: 'block-transition-controls' }, button('Finish block', () => practice.finishBlock(), 'secondary', 'check'), button('Skip block', () => practice.finishBlock(true), 'ghost', 'skip'), button('Restart block', async () => { await practice.restart(); notify('New segment ready. Previous time and attempts remain in history.', 'info'); }, 'ghost', 'restart'));
    const main = el('section', { class: 'practice-workspace' }, el('div', { class: 'practice-identity' }, el('div', { class: 'practice-state' }, blockNumber, status), title, sticking), el('div', { class: 'practice-readouts' }, el('div', { class: 'time-readout' }, el('span', { class: 'label' }, 'Active time'), el('div', { class: 'active-timer' }, time, target), track), el('div', { class: 'tempo-readout' }, el('label', { class: 'label', for: 'practice-bpm' }, 'Tempo · BPM'), tempo, steps)), beats, el('div', { class: 'practice-main-controls' }, start, metro), el('div', { class: 'attempt-section' }, el('div', { class: 'label' }, 'Record this attempt'), el('div', { class: 'rating-buttons' }, ratingButtons), attemptText), transition, el('div', { class: 'practice-tools' }, button('Quick note', note, 'ghost', 'note'), button('Tempo trainer', () => trainerDialog(practice.session.blocks[practice.session.activeBlockIndex].tempoTrainer, config => practice.trainer(config), practice.session.runtime.bpm), 'ghost', 'progress')), progressText, notesText, error, next);
    ...
    const text = (node, value) => { if (node.textContent !== value)
    ...
    const tick = () => {
        ...
        text(time, clock(elapsed));
        text(target, `of ${clock(block.targetSeconds)}`);
        const width = `${Math.min(100, elapsed / block.targetSeconds * 100).toFixed(2)}%`;
        if (fill.style.width !== width)
            fill.style.width = width;
        if (block.tempoTrainer)
            text(progressText, trainerLabel(block.tempoTrainer, Math.max(0, elapsed - session.runtime.trainerStartSeconds), session.runtime.trainerCleanRounds));
        else
            text(progressText, elapsed >= block.targetSeconds ? 'Target time reached. Finish when you are ready.' : '');
    };
    function draw() {
        const session = practice.session;
        if (!session)
        ...
        if (lastState !== state) {
            ...
            text(sticking, block.stickingSnapshot);
            ...
            text(metro.querySelector('span'), session.runtime.metronomeOn ? 'Metronome on' : 'Metronome off');
            ...
            text(error, practice.error);
            text(notesText, block.notes);
        ...
        if (lastIndex !== session.activeBlockIndex || lastId !== block.id) {
            ...
            next.replaceChildren(el('span', { class: 'label' }, upcoming ? 'Up next' : 'Final block'), el('strong', {}, upcoming ? `${upcoming.titleSnapshot} · ${duration(upcoming.targetSeconds)}` : 'Finish the block to review your session.'));
    ...
    const unsubscribe = practice.subscribe(draw), timer = setInterval(tick, 250);
    draw();
