...
export function libraryPage() {
    ...
    const search = el('input', { type: 'search', placeholder: 'Search exercises or tags…', 'aria-label': 'Search exercises' });
    ...
    const results = el('div', { class: 'exercise-grid' }), count = el('span', { class: 'muted small' });
    let mode = 'grid';
    const render = () => {
        ...
        count.textContent = `${exercises.length} exercises`;
        results.className = mode === 'grid' ? 'exercise-grid' : 'exercise-list';
        results.replaceChildren();
        if (!exercises.length) {
            results.append(empty('No exercises match.', 'Try another filter, or create an exercise for what you are working on.', button('New exercise', () => editExercise(), 'primary', 'plus'), 'library'));
            return;
        }
        for (const e of exercises) {
            const best = calculateBestCleanBpm(exerciseAttempts(data.sessions, e.id));
            results.append(el('article', { class: 'exercise-card' }, el('div', { class: 'exercise-card-top' }, badge(titleCase(e.category)), el('span', { class: 'muted tiny' }, e.builtin ? 'BUILT-IN' : 'CUSTOM')), el('h2', {}, link(e.name, `/library/${e.id}`)), e.sticking ? el('p', { class: 'sticking small-sticking' }, e.sticking) : el('p', { class: 'muted exercise-description' }, e.description), el('div', { class: 'exercise-card-bottom' }, el('div', {}, el('span', { class: 'muted tiny' }, 'BEST CLEAN'), el('strong', {}, best ? `${best} BPM` : 'Not recorded')), iconButton(`Practice ${e.name}`, 'play', () => launchPractice([exerciseBlock(e)])))));
        }
    };
...
export function exercisePage(id) {
    ...
    if (!history.length)
        historySection.append(empty('Your baseline starts here.', 'Finish a practice session and record an attempt to see it here.', button('Practice this exercise', () => launchPractice([exerciseBlock(exercise)]), 'ghost', 'play'), 'history'));
    else
