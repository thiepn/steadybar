...
export function libraryPage() {
    ...
    category.value = libraryView.category;
    source.value = libraryView.source;
    ...
    const render = () => {
        ...
        const exercises = indexed.filter(({ exercise: e, text }) => (source.value === 'archived' ? e.archived : !e.archived) && (!category.value || e.category === category.value) &&
        ...
        const rows = [];
        if (!exercises.length)
            rows.push(empty('No matching exercises', 'Change a filter or add an exercise.', button('New exercise', () => editExercise(), 'secondary', 'plus')));
        for (const e of exercises.slice(0, libraryView.visible)) {
            const best = clean.get(e.id);
            rows.push(el('article', { class: 'exercise-card' }, el('div', { class: 'exercise-card-top' }, badge(titleCase(e.category)), el('span', { class: 'muted tiny' }, e.archived ? 'Archived' : e.builtin ? 'Built-in' : 'Custom')), el('h2', {}, link(e.name, `/library/${e.id}`)), e.sticking ? el('p', { class: 'sticking small-sticking' }, e.sticking) : el('p', { class: 'muted exercise-description' }, e.description), el('div', { class: 'exercise-card-bottom' }, el('div', {}, el('span', { class: 'muted tiny' }, best ? 'Best clean' : 'Starting tempo'), el('strong', {}, `${best || e.defaultBpm} BPM`)), iconButton(`Practice ${e.name}`, 'play', () => launchPractice([exerciseBlock(e)])))));
        }
        results.replaceChildren(...rows);
    };
    for (const control of [search, category, source, sort])
        control.addEventListener(control === search ? 'input' : 'change', () => { libraryView.visible = 60; render(); });
    const viewButtons = ['List', 'Grid'].map(label => {
        ...
        b.dataset.view = label.toLowerCase();
        b.setAttribute('aria-pressed', String(libraryView.mode === b.dataset.view));
        return b;
    });
    render();
    return { node: el('div', { class: 'page library-page' }, pageHeader('', 'Exercise library', 'Exercises, sticking patterns, and tempo records.', [button('New exercise', () => editExercise(), 'primary', 'plus')]), el('div', { class: 'library-toolbar' }, el('div', { class: 'search-field' }, search), category, source, sort), el('div', { class: 'split result-meta' }, count, el('div', { class: 'segmented', role: 'group', 'aria-label': 'Exercise view' }, viewButtons)), results, el('div', { class: 'page-footer' }, more)) };
}
export function exercisePage(id) {
    const data = store.snapshot(), exercise = data.exercises.find(e => e.id === id);
    if (!exercise)
        return { node: empty('Exercise not found.', 'It may have been removed by a restore. Your historical session snapshots are independent.', link('Back to library', '/library', 'button primary')) };
    ...
    target.addEventListener('change', async () => { if (!target.reportValidity())
        return; try {
        await store.save('exercises', { ...exercise, targetBpm: target.value ? Number(target.value) : undefined });
        notify('Clean-BPM target saved.');
    }
    catch (e) {
        notify(e instanceof Error ? e.message : 'Target could not be saved.', 'error');
    } });
    ...
    if (exercise.archived)
        page.append(el('div', { class: 'info-banner' }, 'This exercise is archived. Its practice history is preserved.', button('Restore to library', async () => { await store.save('exercises', { ...exercise, archived: false }); }, 'ghost')));
    if (exercise.sticking)
        page.append(el('section', { class: 'sticking-panel' }, el('div', { class: 'label' }, 'STICKING'), el('div', { class: 'sticking' }, exercise.sticking), el('p', { class: 'muted small' }, 'R = right · L = left · K = kick · lowercase = grace stroke. Sticking describes hand order, not full rhythmic notation.')));
