...
export function routinesPage() {
    ...
    const draw = () => {
        ...
        if (!routines.length)
            list.append(empty(showArchivedRoutines ? 'No archived routines.' : 'No routines yet', 'Build a sequence to use again.', button('New routine', () => editRoutine(), 'secondary', 'plus')));
        for (const r of routines) {
            ...
            list.append(el('article', { class: 'routine-card' }, el('div', { class: 'routine-row' }, el('div', { class: 'routine-identity' }, el('h2', {}, link(r.name, `/routines/${r.id}`)), el('p', { class: 'muted small' }, r.description || `${r.blocks.length} practice blocks`), el('p', { class: 'routine-schedule muted tiny' }, `${r.builtin ? 'Starter' : 'Custom'}${r.scheduledDays.length ? ` · ${r.scheduledDays.map(d => ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][d]).join(', ')}` : ''}`)), el('div', { class: 'routine-duration' }, el('strong', {}, duration(routineDuration(r.blocks))), el('span', { class: 'muted tiny' }, `${r.blocks.length} blocks`)), play), el('details', { class: 'routine-disclosure' }, el('summary', {}, 'Preview sequence'), el('ol', { class: 'routine-preview' }, r.blocks.map(b => el('li', {}, el('span', {}, b.title), el('span', { class: 'muted' }, `${duration(b.targetSeconds, true)} · ${b.bpm} BPM`)))), link('Open routine', `/routines/${r.id}`, 'text-link', 'arrow'))));
        }
    };
