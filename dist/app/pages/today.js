...
export function todayPage() {
    ...
    if (active)
        page.append(el('div', { class: 'recovery-banner' }, el('div', {}, el('strong', {}, 'You have an unfinished session.'), el('span', {}, active.blocks[active.activeBlockIndex]?.titleSnapshot || 'Your practice is saved.')), link('Resume session', '/practice/active', 'button primary', 'play')));
    ...
    if (plan?.blocks.length)
        planPanel.append(blockList(plan.blocks, savePlan, block => launchPractice([block], { planId: plan.id })), el('div', { class: 'plan-footer' }, el('span', { class: 'muted small' }, 'Your plan is saved on this device.'), start));
    else
    ...
    const quick = el('section', { class: 'panel quiet-panel' }, sectionHeader('Find your focus'), el('div', { class: 'quick-links' }, link('Open metronome', '/metronome', 'quick-link', 'pulse'), link('Work on a rudiment', '/library', 'quick-link', 'library'), link('Prepare a song', '/songs', 'quick-link', 'song'), link('Browse routines', '/routines', 'quick-link', 'routine')));
    ...
    if (!relevant.length)
        goals.append(empty('Set a direction.', 'Choose a clean tempo, a song to prepare, or a sustainable weekly rhythm.', button('Create a goal', () => editGoal(), 'ghost', 'plus'), 'goal'));
    else
        goals.append(...relevant.map(g => { const p = goalProgress(g, data); return el('div', { class: 'goal-summary' }, el('strong', {}, g.title), el('div', { class: 'split muted small' }, el('span', {}, p.label), g.deadline ? el('span', {}, formatDate(g.deadline)) : null), progressBar(p.fraction, g.title)); }));
    ...
    if (!recent.length)
        history.append(empty('A fresh practice record.', 'Your sessions will appear here once you finish your first practice.', button('Start a short session', () => launchPractice([freeBlock(300, data.settings.metronome.bpm)]), 'ghost', 'play'), 'history'));
    else
        history.append(...recent.map(s => link(`${formatDate(s.startedAt)} · ${duration(sessionTime(s))} · ${s.blocks[0]?.titleSnapshot || 'Practice'}`, `/history/${s.id}`, 'recent-row', 'history')));
    page.append(el('div', { class: 'two-column' }, goals, history));
    return { node: page };
}
