...
export function metronomePage() {
    ...
    const toggle = async () => {
        ...
        await audio.start(config, { onBeat: event => { if (disposed)
                return; const label = event.countingIn ? `Count-in · bar ${event.bar + 1}` : 'Playing'; if (status.textContent !== label)
                status.textContent = label; Array.from(beats.children).forEach((b, i) => b.classList.toggle('on', i === event.beat)); }, onInterrupted: () => { stop(); notify('Audio was suspended. Tap Start to resume.', 'info'); } });
        if (disposed) {
            audio.stop();
            return;
        }
        running = true;
        status.textContent = config.countIn ? 'Count-in' : 'Playing';
    ...
    const main = el('section', { class: 'panel metronome-main' }, el('div', { class: 'instrument-heading' }, el('label', { class: 'label', for: 'metronome-bpm' }, 'Tempo · BPM'), status), el('div', { class: 'tempo-display' }, tempo), el('div', { class: 'standalone-tempo-steps' }, [-10, -5, -1, 1, 5, 10].map(step => button(step > 0 ? `+${step}` : `−${Math.abs(step)}`, () => setBpm(config.bpm + step), 'tempo-step'))), slider, el('div', { class: 'metronome-transport' }, play, el('div', { class: 'tap-tempo' }, tap, tapCount)), el('div', { class: 'meter-workspace' }, el('div', { class: 'metronome-config' }, field('Time signature', signature), field('Subdivision', subdivision), field('Count-in', countIn)), beats, el('p', { class: 'muted small accent-help' }, 'Tap a beat: accent → normal → mute.'), meterNote), el('div', { class: 'volume-row' }, field('Volume', volume), volumeLabel, button('Custom meter', customMeter, 'ghost')));
    tempo.id = 'metronome-bpm';
    const saved = el('aside', { class: 'metronome-side' }, el('section', { class: 'panel' }, sectionHeader('Presets', undefined, [button('Save current', () => savePreset(config), 'ghost', 'plus')]), presets), el('details', { class: 'playback-help' }, el('summary', {}, 'Playback & shortcuts'), el('p', { class: 'muted small' }, 'The sound sets the timing; the visual pulse follows it. Keep this app in the foreground for dependable playback.'), el('dl', { class: 'shortcut-list' }, el('dt', {}, 'Start / pause'), el('dd', {}, el('kbd', {}, 'Space')), el('dt', {}, 'Change tempo'), el('dd', {}, el('kbd', {}, '↑ ↓')), el('dt', {}, 'Change by 5 BPM'), el('dd', {}, el('kbd', {}, 'Shift + ↑ ↓')))));
    const drawPresets = () => {
