...
export function songsPage() {
    ...
    const draw = () => {
        songView.query = search.value;
        songView.status = status.value;
        ...
        more.hidden = songs.length <= songView.visible;
        if (!songs.length)
            list.append(empty('No songs found', 'Add a song, then break it into sections for practice.', button('Add song', () => editSong(), 'secondary', 'plus')));
        for (const s of songs.slice(0, songView.visible))
            list.append(el('article', { class: 'song-card song-row' }, el('div', { class: 'repertoire-identity' }, el('h2', {}, link(s.title, `/songs/${s.id}`)), s.artist ? el('p', { class: 'muted small' }, s.artist) : null), el('div', { class: 'repertoire-arrangement' }, el('span', { class: 'song-status', 'data-status': s.status }, titleCase(s.status)), el('span', { class: 'muted tiny' }, `${s.meter.beats}/${s.meter.beatUnit}${s.key ? ` · ${s.key}` : ''} · ${s.sections.length} sections`)), el('div', { class: 'song-tempo' }, el('strong', {}, s.bpm), el('small', {}, 'BPM')), iconButton(`Practice ${s.title}`, 'play', () => launchPractice([songBlock(s)]))));
    };
    search.addEventListener('input', () => { songView.visible = 60; draw(); });
    status.addEventListener('change', () => { songView.visible = 60; draw(); });
    draw();
    return { node: el('div', { class: 'page' }, pageHeader('', 'Songs', 'Repertoire, sections, and arrangement notes.', [button('Add song', () => editSong(), 'primary', 'plus')]), el('div', { class: 'library-toolbar' }, search, status), el('div', { class: 'result-meta' }, count), list, el('div', { class: 'page-footer' }, more)) };
}
export function songPage(id) {
    ...
    if (!song.sections.length)
        sections.append(empty('No sections yet', 'Add an intro, verse, chorus, bridge, or any section that needs focused practice.', button('Add first section', () => editSection(song), 'ghost', 'plus'), 'song'));
    let dragIndex = -1;
