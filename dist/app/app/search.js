...
export function openSearch() {
    ...
    const handle = dialog('Search', [query, results, announcement, el('p', { class: 'command-hint muted tiny' }, '↑ ↓ to move · Enter to open · Esc to close')]);
    handle.dialog.classList.add('command-dialog');
    const highlight = () => { announcement.textContent = buttons[selected] ? `${buttons[selected].textContent}. Result ${selected + 1} of ${buttons.length}.` : ''; buttons.forEach((b, i) => { b.classList.toggle('highlighted', i === selected); b.setAttribute('aria-current', String(i === selected)); if (i === selected)
        b.scrollIntoView({ block: 'nearest' }); }); };
    const render = () => {
        const q = query.value.trim().toLowerCase(), filtered = indexed.filter(item => item.searchText.includes(q)).slice(0, 30);
        selected = 0;
