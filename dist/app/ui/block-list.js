...
export function blockList(blocks, onChange, onStart) {
    ...
    blocks.forEach((block, index) => {
        ...
        const options = () => {
            ...
            const action = (label, accessible, symbol, run, disabled = false) => {
        ...
        if (block.type === 'exercise') {
            const exercise = store.snapshot().exercises.find(e => e.id === block.exerciseId);
            if (exercise?.sticking)
                title.append(el('p', { class: 'block-cue sticking' }, exercise.sticking));
        }
        list.append(row);
    });
