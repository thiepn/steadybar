...
export function blockList(blocks, onChange, onStart) {
    ...
    let dragging = -1;
    const change = async (next) => { await onChange(ordered(next)); };
    blocks.forEach((block, index) => {
        ...
        const updateInput = async (field, value, control) => {
            if (!control.reportValidity())
                return;
            try {
                await change(blocks.map(b => b.id === block.id ? { ...b, [field]: value, ...(field === 'targetSeconds' && b.tempoTrainer?.mode === 'endurance' ? { tempoTrainer: { ...b.tempoTrainer, seconds: value } } : {}) } : b));
            }
            catch (error) {
        ...
        const up = iconButton(`Move ${block.title} up`, 'up', () => change(reorder(blocks, index, index - 1)));
        up.disabled = index === 0;
        const down = iconButton(`Move ${block.title} down`, 'down', () => change(reorder(blocks, index, index + 1)));
        down.disabled = index === blocks.length - 1;
        const row = el('div', { class: 'block-row', 'data-block-id': block.id }, el('div', { class: 'block-index' }, el('span', {}, String(index + 1).padStart(2, '0')), el('span', { class: 'drag-handle', draggable: true, title: 'Drag to reorder', 'aria-hidden': 'true', onDragstart: (event) => { dragging = index; event.dataTransfer?.setData('text/plain', block.id); row.classList.add('dragging'); }, onDragend: () => row.classList.remove('dragging') }, icon('grip', 16))), title, el('div', { class: 'block-numbers' }, el('label', {}, minutes, el('span', {}, 'min')), el('label', {}, bpm, el('span', {}, 'BPM'))), el('div', { class: 'block-actions' }, onStart ? iconButton(`Practice ${block.title}`, 'play', () => onStart(block)) : null, iconButton(`Edit ${block.title}`, 'edit', () => editBlock(block, async (updated) => change(blocks.map(b => b.id === block.id ? updated : b)))), iconButton(`Tempo trainer for ${block.title}`, 'progress', () => trainerDialog(block.tempoTrainer, async (config) => change(blocks.map(b => b.id === block.id ? { ...b, tempoTrainer: config, ...(config.mode === 'endurance' ? { targetSeconds: config.seconds } : {}) } : b)), block.bpm)), up, down, iconButton(`Remove ${block.title}`, 'close', () => change(blocks.filter(b => b.id !== block.id)))));
        row.addEventListener('dragover', event => { event.preventDefault(); if (event.dataTransfer)
            event.dataTransfer.dropEffect = 'move'; });
        row.addEventListener('drop', event => { event.preventDefault(); if (dragging >= 0)
            void change(reorder(blocks, dragging, index)).catch(error => notify(error instanceof Error ? error.message : 'Reordering could not be saved.', 'error')); dragging = -1; });
        list.append(row);
    });
