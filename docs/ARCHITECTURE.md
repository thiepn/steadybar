...
Database name: `music-practice-os`. Current IndexedDB version: **5**. Stores: profiles, courseProgress, exercises, songs, routines, dailyPlans, sessions, goals, setlists, practiceStates, priorityCycles, metronomePresets, settings. `migrationBackups` remains an internal migration-safety store.

Version 1 created the original workspace stores. Version 2 introduced the background-pause preference, version 3 added practice profiles and profile-scoped plans, version 4 added guided-course progress, and version 5 adds rebuildable practice state plus priority cycles. Migrations do not reinterpret immutable session history. New migrations belong in `db/database.ts` with explicit transforms and native-browser upgrade coverage.
...
Backup envelope version **4** is separate from IndexedDB version 5. The document contains format, version, export timestamp, the complete modern workspace, guided-course progress, practice state and priority cycles; versions 1–3 remain importable through explicit migrations. Export reads one consistent snapshot transaction. Import validates the whole document before clear-and-put replacement in one multi-store transaction. The database replacement entry point itself validates every record before opening the destructive transaction, not just the import dialog. Restored active sessions are paused immediately at the saved checkpoint. The UI supports replacement only; there is deliberately no partial merge policy.
