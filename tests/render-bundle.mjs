...
let data;
const validators={exercises:v.validateExercise,songs:v.validateSong,routines:v.validateRoutine,dailyPlans:v.validatePlan,sessions:v.validateSession,goals:v.validateGoal,setlists:v.validateSetlist,metronomePresets:v.validatePreset,settings:v.validateSettings};
exports.initializeDatabase=async()=>{data??=seed();};
...
exports.put=async(name,value)=>{const next=validators[name](value);if(name==='settings'){data.settings=next;return;}const i=data[name].findIndex(x=>x.id===next.id);if(i<0)data[name].push(next);else data[name][i]=next;};
