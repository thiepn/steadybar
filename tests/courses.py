...
class Courses(e2e.MusicPracticeTests):
    ...
    def test_90_native_database_three_to_five_preserves_old_practice(self):
        ...
        self.assertEqual(data['courseProgress'],[]);self.assertEqual(data['sessions'][0],result['session']);self.assertEqual(self.read("load('db/database.js').openDatabase().then(d=>d.version)"),7)
