...
class Profiles(e2e.MusicPracticeTests):
    ...
    def test_63_vocal_range_reference_and_fatigue_pause(self):
        ...
        self.save_dialog('Save review')
        self.wait_read("load('practice/controller.js').practice.session.runtime.phase",lambda phase:phase=='paused')
        expect(self.page.locator('.focus-start')).to_have_attribute('aria-label','Resume practice')
    ...
    def test_70_native_upgrade_preserves_original_database_and_exports_safety_copy(self):
        ...
        self.assertEqual(len(self.read("load('app/store.js').store.view().exercises")),36)
        self.assertEqual(self.page.evaluate("indexedDB.databases().then(d=>d.find(d=>d.name==='music-practice-os').version)"),11)
        self.route('/settings')
