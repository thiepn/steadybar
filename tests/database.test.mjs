...
const adapter=transactionAdapter(db.STORES);
...
const active=()=>createSession([free()],seedData());
...
test('repository initialization commits all starter tables without fake history',async()=>{
  ...
  assert.equal(data.exercises.length,30);assert.equal(data.routines.length,4);assert.equal(data.sessions.length,0);
...
test('repository exercise create/read/update/archive uses durable repository calls',async()=>{
  await db.initializeDatabase();const e={...seedData().exercises[0],...metadata(),name:'My control exercise',builtin:false};
...
test('repository does not report save completion after a request but before commit',async()=>{
  ...
  const e={...seedData().exercises[0],...metadata(),name:'Commit barrier'};let resolved=false;
  const write=db.put('exercises',e).then(()=>{resolved=true;});
  for(let i=0;i<12&&!adapter.state.held.length;i++)await flush();
...
test('repository rolls back a failed commit and surfaces the original error',async()=>{
  await db.initializeDatabase();const e={...seedData().exercises[0],...metadata(),name:'Not saved'};
...
test('complete backup restore preserves all entity types, attempts and historical snapshots',async()=>{
  ...
  const s=active();s.blocks[0].sourceExerciseId=e.id;s.blocks[0].titleSnapshot=e.name;s.blocks[0].actualActiveSeconds=120;
  ...
  assert.deepEqual(await db.readData(),exported.data);
});
