...
for(const type of Object.keys(baseCounts)){
 ...
 test(`${type}: every starter task can enter a session, finish, export and restore`,()=>{
  ...
  const b=createBackup(d);assert.equal(b.version,3);assert.deepEqual(parseBackup(JSON.stringify(b)).data,validateData({...d,courseProgress:[],trainingPlans:[],weeklySchedules:[],recordings:[],timingResults:[],midiDeviceProfiles:[],midiResults:[]}));
 });
