...
class MusicPracticeTests(unittest.TestCase):
    ...
    def read(self,expression):
        ...
        return self.page.evaluate("async (expr)=>{const names=['app/store.js','practice/controller.js','db/database.js','db/backup.js','db/media.js','domain/analytics.js','audio/engine.js','audio/track-player.js','domain/models.js','practice/logic.js','db/seed.js','app/profiles.js','audio/reference.js','domain/protocols.js','domain/drum-phrase.js','domain/midi-analysis.js','domain/utils.js','learning/catalog.js','learning/engine.js','ui/editors.js'];const entries=await Promise.all(names.map(async id=>[id,await import('./app/'+id)]));const modules=Object.fromEntries(entries);return await (new Function('load','return ('+expr+')'))(id=>modules[id]);}",expression)
    ...
    def open_focus_drawer(self,label):
        summary=self.page.locator('details > summary').filter(has_text=label).first
    ...
    def test_09_pause_and_trainer_modes(self):
        ...
        for _ in range(3):self.open_focus_drawer('Detailed attempt');self.page.get_by_role('button',name='Clean',exact=True).click()
        self.finish()
    ...
    def test_11_backup_validation_ui(self):
        ...
        self.assertEqual(self.read("load('app/store.js').store.snapshot().exercises.length"),39)
