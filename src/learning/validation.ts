...
export const validateCourseProgress:Validator<CourseProgress>=(value,path='Course progress')=>{
  ...
  for(const l of p.lessons)for(const a of l.attempts){
    ...
    if(course&&a.revision===course.revision){
      if(!lesson)fail(path,'the review does not match this lesson revision');
      if(a.checks.length!==lesson!.checks.length||a.answers.length!==lesson!.questions.length)fail(path,'the review does not match this lesson revision');
