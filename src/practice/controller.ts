...
export class PracticeController {
  ...
  private report(error:unknown):void{this.generation++;this.error=error instanceof Error ? error.message : 'Practice data could not be saved. Check browser storage permissions.';audio.stop();this.stopTimers();this.unlock();if(this.session)this.session=recoverSession(this.session);this.emit();}
  ...
  async create(blocks:RoutineBlock[],source:{routineId?:string;planId?:string}={}):Promise<void>{
  ...
  async start():Promise<void>{
    ...
    const generation=++this.generation;this.error='';await this.lock();
  ...
  async pause():Promise<void>{
    this.generation++;audio.stop();this.stopTimers();
  ...
  async trainer(config:TrainerConfig | undefined):Promise<void>{
    await this.pause();await this.mutate(s=>{const block=s.blocks[s.activeBlockIndex]!;block.tempoTrainer=config;s.runtime.trainerStartSeconds=block.actualActiveSeconds;s.runtime.trainerCleanRounds=0;if(config){s.runtime.bpm=trainerBpm(config,0,0);block.finalBpm=s.runtime.bpm;if(config.mode==='endurance')block.targetSeconds=Math.ceil(block.actualActiveSeconds)+config.seconds;}return s;});
  }
  async finishBlock(skip=false):Promise<void>{this.generation++;audio.stop();this.stopTimers();await this.mutate(s=>finishBlock(s,skip));this.beat=undefined;this.emit();}
  async restart():Promise<void>{this.generation++;audio.stop();this.stopTimers();await this.mutate(restartBlock);this.beat=undefined;this.emit();}
  async finish(abandon=false):Promise<void>{
    this.generation++;this.recovered=false;audio.stop();this.stopTimers();await this.mutate(s=>{
      s=pauseSession(s);const now=nowISO();
      s.blocks.forEach((b,i)=>{if(i===s.activeBlockIndex){b.completed=!abandon;b.endedAt=now;b.finalBpm=s.runtime.bpm;}else if(i>s.activeBlockIndex){b.skipped=true;b.endedAt=now;}});
      s.status=abandon?'abandoned':'completed';s.endedAt=now;return s;
    });
  }
  async discard():Promise<void>{this.generation++;audio.stop();this.stopTimers();await this.lock();try{if(this.session){await store.delete('sessions',this.session.id);this.session=undefined;this.recovered=false;this.emit();}}finally{this.unlock();}}
