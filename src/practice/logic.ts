...
export function createSession(blocks:RoutineBlock[],data:Data,source:{routineId?:string;planId?:string}={}):PracticeSession {
...
export function finishBlock(session:PracticeSession,skip=false,now=Date.now()):PracticeSession {
  ...
  block.completed=!skip;block.skipped=skip;block.endedAt=new Date(now).toISOString();block.finalBpm=next.runtime.bpm;
...
export function restartBlock(session:PracticeSession,now=Date.now()):PracticeSession {
  ...
  const fresh:PracticeBlock={...structuredClone(old),id:uuid(),actualActiveSeconds:0,initialBpm:startBpm,finalBpm:startBpm,tempoAttempts:[],startedAt:undefined,endedAt:undefined,completed:false,skipped:false};
