...
export function snapshotBlock(block:RoutineBlock,data:Data):PracticeBlock {
  ...
  if(protocol && 'pulse' in protocol && protocol.pulse && block.progression?.subdivision!==undefined)protocol.pulse.subdivision=block.progression.subdivision;
...
export function restartBlock(session:PracticeSession,now=Date.now()):PracticeSession {
  ...
  const fresh:PracticeBlock={...structuredClone(old),id:uuid(),actualActiveSeconds:0,initialBpm:old.initialBpm===undefined?undefined:startBpm,finalBpm:old.initialBpm===undefined?undefined:startBpm,tempoAttempts:[],evaluation:undefined,...(old.protocolSnapshot?{outcomes:[],protocolState:{step:0,clean:0,total:0,...(old.protocolSnapshot.kind==='vocal-pattern'?{rootMidi:old.protocolSnapshot.startMidi}:{})}}:{}),startedAt:undefined,endedAt:undefined,completed:false,skipped:false};
