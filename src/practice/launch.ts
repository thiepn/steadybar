...
export function exerciseBlock(exercise:Exercise,seconds=600):RoutineBlock{return {id:uuid(),type:'exercise',exerciseId:exercise.id,title:exercise.name,targetSeconds:seconds,bpm:exercise.defaultBpm,notes:'',order:0};}
export function songBlock(song:Song,sectionId?:string,seconds=600):RoutineBlock{
...
export async function launchPractice(blocks:RoutineBlock[],source:{routineId?:string;planId?:string}={}):Promise<void>{
  ...
  await practice.create(blocks,source);navigate('/practice/active');
}
export async function addToday(block:RoutineBlock):Promise<void>{
  ...
  plan.blocks.push({...structuredClone(block),id:uuid(),order:plan.blocks.length});await store.save('dailyPlans',plan);notify('Added to today’s plan.');
}
export async function routineToday(routine:Routine):Promise<void>{
  ...
  await store.save('dailyPlans',{...(current||metadata()),date:localDate(),sourceRoutineId:routine.id,blocks:freshBlocks(routine.blocks)});notify('Today’s plan is ready.');navigate('/');
}
