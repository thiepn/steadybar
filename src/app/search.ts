...
export function openSearch():void{
  ...
  const data=store.snapshot(),selectedProfileId=activeProfile(data).id,availableProfileIds=new Set(practiceProfiles(data).map(p=>p.id)),query=el('input',{type:'search',placeholder:'Find a lesson, exercise, song, recording, MIDI tool, routine, or action…','aria-label':'Search everything',autocomplete:'off',class:'command-input'}),results=el('div',{class:'command-results',role:'list','aria-label':'Search results'});
  const items:SearchItem[]=[
    {label:'Start practice',type:'Command',icon:'play',action:()=>navigate('/practice')},{label:'Open metronome',type:'Command',icon:'pulse',action:()=>navigate('/metronome')},{label:'Open Timing Lab',type:'Command',icon:'pulse',action:()=>navigate('/timing-lab')},{label:'Open MIDI Drum Lab',type:'Command',icon:'pulse',action:()=>navigate('/midi-lab')},
