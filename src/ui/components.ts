...
export function notify(message:string,kind:'success'|'error'|'info'='success'):void{
  ...
  // Keep repeated saves quiet, especially on a phone beside an instrument.
  if(kind!=='error')for(const previous of root.querySelectorAll('.toast:not(.error)'))previous.remove();
  note.append(close);root.append(note);setTimeout(()=>note.remove(),kind==='error'?14000:3500);
}
...
export function input(name:string,label:string,value:string|number='',type='text',attrs:Record<string,unknown>={}):HTMLElement{return field(label,el('input',{name,type,value,...attrs}));}
export function textarea(name:string,label:string,value='',rows=3):HTMLElement{return field(label,el('textarea',{name,rows,maxlength:10000},value));}
export function select(name:string,label:string,options:(string|[string,string])[],value:string,hint?:string):HTMLElement{
  const control=el('select',{name},options.map(option=>{const [v,text]=Array.isArray(option)?option:[option,option];return el('option',{value:v,selected:v===value},text);}));return field(label,control,hint);
}
export function checkbox(name:string,label:string,checked=false):HTMLElement{return el('label',{class:'checkbox'},el('input',{type:'checkbox',name,checked}),el('span',{},label));}
export function progressBar(fraction:number,label:string):HTMLElement{return el('div',{class:'progress-track',role:'progressbar','aria-label':label,'aria-valuenow':Math.round(Math.min(1,Math.max(0,fraction))*100),'aria-valuemin':0,'aria-valuemax':100},el('div',{class:'progress-fill',style:`width:${Math.min(1,Math.max(0,fraction))*100}%`}));}
export interface DialogHandle {dialog:HTMLDialogElement;close:()=>void}
export function dialog(title:string,content:Child[],footer:Child[]=[],onClosed?:()=>void):DialogHandle{
  const before=document.activeElement as HTMLElement|null;
  const headingId=`dialog-${uuid()}`;
  const d=el('dialog',{'aria-labelledby':headingId,class:'dialog'});
  let closing=false;
  const close=()=>{if(closing)return;closing=true;d.close();d.remove();if(before?.isConnected)before.focus();onClosed?.();};
  ...
  if(footer.length)d.append(el('div',{class:'dialog-footer'},footer));
  d.addEventListener('cancel',event=>{event.preventDefault();close();});
  document.querySelector('#dialogs')!.append(d);d.showModal();
  // showModal has attached and activated the dialog. Focus now, not on a later
  // task which could steal focus after the user has already chosen another field.
  const target=d.querySelector<HTMLElement>('[autofocus],input:not([type=hidden]),select,textarea');(target||closeButton).focus();
  return {dialog:d,close};
}
export async function confirmAction(title:string,description:string,confirm='Confirm',danger=false):Promise<boolean>{
  return new Promise(resolve=>{
    let settled=false;const settle=(value:boolean)=>{if(settled)return;settled=true;resolve(value);handle.close();};
    const handle=dialog(title,[el('p',{},description)],[button('Cancel',()=>settle(false)),button(confirm,()=>settle(true),danger?'danger':'primary')],()=>{if(!settled)resolve(false);});
  });
}
export function formDialog(title:string,children:Child[],onSave:(data:FormData,form:HTMLFormElement)=>Promise<unknown>,submitLabel='Save'):void{
  let dirty=false,saving=false;const form=el('form',{class:'form'},children),error=el('p',{class:'form-error',role:'alert'});
  const save=el('button',{type:'submit',class:'button primary'},submitLabel);
  const attemptClose=async()=>{if(saving)return;if(!dirty || await confirmAction('Discard unsaved changes?','Your edits have not been saved.','Discard changes',true))handle.close();};
  const handle=dialog(title,[form]);
  const oldClose=handle.dialog.querySelector('.dialog-header button');oldClose?.replaceWith(iconButton('Close dialog','close',attemptClose));
  ...
  form.addEventListener('submit',async event=>{
    event.preventDefault();if(!form.reportValidity()||saving)return;error.textContent='';save.disabled=true;saving=true;
    try{await onSave(new FormData(form),form);dirty=false;handle.close();}
    catch(e){error.textContent=errorMessage(e);error.scrollIntoView({block:'nearest'});}
    finally{save.disabled=false;saving=false;}
  });
}
export const formText=(data:FormData,name:string):string=>String(data.get(name)??'').trim();
export const formNumber=(data:FormData,name:string):number=>Number(formText(data,name));
