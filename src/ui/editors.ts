...
export function editExercise(exercise?:Exercise):void{
  const e=exercise||{...metadata(),name:'',instrument:store.snapshot().settings.instrument,category:'technique' as const,description:'',instructions:'',sticking:'',accents:'',defaultBpm:80,targetBpm:120,minBpm:20,maxBpm:300,meter:{beats:4,beatUnit:4 as const},subdivision:1 as const,tags:[],notes:'',builtin:false,archived:false};
  formDialog(exercise?'Edit exercise':'New exercise',[
    ...
    await store.save('exercises',saved);notify(exercise?'Exercise saved.':'Exercise created.');if(!exercise)navigate(`/library/${saved.id}`);
  },'Save exercise');
}
...
export function editSection(song:Song,section?:SongSection):void{
  ...
  formDialog(section?'Edit section':'New song section',[
    ...
    await store.save('songs',updated);notify('Section saved.');
  },'Save section');
}
...
export function editGoal(goal?:Goal):void{
  const data=store.snapshot();const g=goal||{...metadata(),type:'bpm' as const,title:'',description:'',targetValue:120,unit:'BPM',completed:false};
  ...
  const update=()=>{const type=typeField.querySelector('select')!.value;exField.hidden=type!=='bpm';songField.hidden=type!=='song-mastery';target.hidden=!['bpm','weekly-sessions'].includes(type);target.querySelector('input')!.required=!target.hidden;if(!goal)target.querySelector('input')!.value=type==='weekly-sessions'?'4':'120';};
  typeField.addEventListener('change',update);update();
  formDialog(goal?'Edit goal':'New goal',[
    ...
    if(type==='song-mastery'&&!formText(form,'songId'))throw new Error('Add a song before creating a song-preparation goal.');
    if(type==='bpm'&&!formText(form,'exerciseId'))throw new Error('Select an exercise for this BPM goal.');
    ...
    if(type==='bpm'&&(saved.targetValue<20||saved.targetValue>300))throw new Error('A BPM target must be between 20 and 300.');
...
export function editBlock(block:RoutineBlock|undefined,onSave:(block:RoutineBlock)=>Promise<unknown>):void{
  const b=block||freeBlock();const data=store.snapshot();
  ...
  const drawSources=()=>{
    const type=typeSelect.querySelector('select')!.value;source.replaceChildren();title.hidden=type!=='free';title.querySelector('input')!.required=type==='free';
    if(type==='exercise'){
      ...
      selection.addEventListener('change',()=>{const e=data.exercises.find(e=>e.id===selection.querySelector('select')!.value);if(e)tempo.querySelector('input')!.value=String(e.defaultBpm);});
      if(!block && options[0])tempo.querySelector('input')!.value=String(options[0].defaultBpm);
    }
    if(type==='song'||type==='song-section'){
      ...
      const drawSections=()=>{const song=data.songs.find(s=>s.id===selection.querySelector('select')!.value);sections.replaceChildren();if(type==='song-section')sections.append(select('songSectionId','Section',(song?.sections||[]).map(s=>[s.id,s.name]),b.songSectionId||song?.sections[0]?.id||''));};
      source.append(selection,sections);selection.addEventListener('change',()=>{drawSections();const song=data.songs.find(s=>s.id===selection.querySelector('select')!.value);if(song)tempo.querySelector('input')!.value=String(song.bpm);});drawSections();
  ...
  formDialog(block?'Edit practice block':'Add practice block',[
    ...
    if(type==='exercise'&&!exercise)throw new Error('Choose an exercise.');if((type==='song'||type==='song-section')&&!song)throw new Error('Choose a song.');if(type==='song-section'&&!section)throw new Error('Choose an existing song section.');
