...
export function blockList(blocks:RoutineBlock[],onChange:(blocks:RoutineBlock[])=>Promise<unknown>,onStart?:(block:RoutineBlock)=>Promise<unknown>):HTMLElement{
  ...
  let draggingId='';
  // UI events can arrive faster than IndexedDB commits (for example blur-save followed
  // immediately by Move). Keep a local ordered working copy and serialize commits so a
  // later action is derived from the latest pending edit rather than the stale render.
  ...
  blocks.forEach((block,index)=>{
    ...
    const up=iconButton(`Move ${block.title} up`,'up',()=>moveById(block.id,-1));up.disabled=index===0;
    const down=iconButton(`Move ${block.title} down`,'down',()=>moveById(block.id,1));down.disabled=index===blocks.length-1;
    const row=el('div',{class:'block-row','data-block-id':block.id},
      ...
      el('div',{class:'block-actions'},onStart?iconButton(`Practice ${block.title}`,'play',()=>onStart(block)):null,
        iconButton(`Edit ${block.title}`,'edit',()=>editBlock(block,async updated=>change(rows=>rows.map(b=>b.id===block.id?updated:b)))),
        iconButton(`Tempo trainer for ${block.title}`,'progress',()=>trainerDialog(block.tempoTrainer,async config=>change(rows=>rows.map(b=>b.id===block.id?{...b,tempoTrainer:config,...(config.mode==='endurance'?{targetSeconds:config.seconds}:{})}:b)),block.bpm)),
        up,down,iconButton(`Remove ${block.title}`,'close',()=>change(rows=>rows.filter(b=>b.id!==block.id)))));
