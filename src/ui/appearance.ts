...
/** Shared controls for Settings and the in-session Appearance dialog. */
export function appearanceControls(): { node: HTMLElement; cleanup: () => void } {
  ...
  for (const p of PALETTES) {
    ...
    b.replaceChildren(el('span', { class: 'palette-preview', 'aria-hidden': 'true' },
      el('span', { class: 'palette-preview-rail' }),
      el('span', { class: 'palette-preview-paper' }, el('span'), el('span'))),
      el('span', { class: 'palette-caption' }, el('span', {}, p.label),
        el('span', { class: 'palette-check', 'aria-hidden': 'true' }, icon('check', 14))));
  ...
  const node = el('div', { class: 'appearance-controls' }, modes, palettes, accents,
    el('div', { class: 'appearance-summary' }, selection, reset), hint);
  const draw = () => {
    ...
    for (const [id, b] of paletteButtons) b.setAttribute('aria-pressed', String(palette === id));
    for (const [id, b] of accentButtons) b.setAttribute('aria-pressed', String(accent === id));
