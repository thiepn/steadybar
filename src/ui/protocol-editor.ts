...
export function protocolEditor(initial:PracticeProtocol,profile:PracticeProfile):ProtocolEditor {
  ...
  switch(p.kind){
    ...
    case 'drum-phrase':{
      node.append(text('phraseName','Phrase name',p.name),textarea('phraseFocus','Practice focus',p.focus,3),el('p',{class:'field-hint'},'Phrase order is fixed as groove bars → fill bar → return bar. Use . for rest, x for hit, X for accent.'));
      p.bars.forEach((bar,index)=>{
        const lane=(voice:string)=>bar.lanes.find(row=>row.voice===voice)?.steps??'.'.repeat(p.pulse.beats*p.pulse.subdivision);
        node.append(el('fieldset',{class:'phrase-bar-editor'},el('legend',{},`${index+1}. ${bar.role.replace(/^./,letter=>letter.toUpperCase())}`),
          text(`phrase-${index}-label`,'Bar label',bar.label),
          el('div',{class:'form-grid'},text(`phrase-${index}-rh`,'Right hand',lane('right-hand')),text(`phrase-${index}-lh`,'Left hand',lane('left-hand'))),
          el('div',{class:'form-grid'},text(`phrase-${index}-kick`,'Kick',lane('kick')),text(`phrase-${index}-foot`,'Hi-hat foot',lane('hihat-foot')))));
      });break;
    }
    case 'drum-dynamics':{
      const lane=(surface:string)=>p.lanes.find(row=>row.surface===surface)?.steps??'.'.repeat(p.pulse.beats*p.pulse.subdivision);
      node.append(
        text('dynName','Dynamics score name',p.name),
        textarea('dynFocus','Practice focus',p.focus,3),
        el('p',{class:'field-hint'},'Use . for rest, 1 for soft/ghost, 2 for medium, and 3 for strong/accent. Levels are relative touch targets, not dB values.'),
        text('dynSnare','Snare levels',lane('snare')),
        text('dynHihat','Hi-hat levels',lane('hihat')),
        text('dynKick','Kick levels',lane('kick')),
        text('dynRide','Ride levels',lane('ride')),
