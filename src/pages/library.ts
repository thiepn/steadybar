...
import { applyExerciseProgression, buildExerciseProgression } from '../domain/progression-engine.js';
...
export function exercisePage(id:string):Page{
  ...
  const progression=buildExerciseProgression(data,exercise);
  ...
  const page=el('div',{class:'page'},link('Exercise library','/library','back-link'),pageHeader(`${skillLabel(exercise.skillArea??exercise.category)} · ${data.profiles?.find(p=>p.id===exercise.profileId)?.name??exercise.instrument}`,exercise.name,exercise.description,[exercise.category==='rudiment'&&exercise.sticking?link('Open Rudiment Lab','/rudiments','button secondary','routine'):null,button('Edit',()=>editExercise(exercise),'secondary','edit'),button('Start practice',()=>launchPractice([exerciseBlock(exercise)]),'primary','play')]));
  ...
  const challengeBlock=()=>applyExerciseProgression(exerciseBlock(exercise),progression);
  page.append(el('section',{class:'panel progression-card'},
    sectionHeader('Next challenge',progression.direction==='advance'?'Ready to increase one variable':progression.direction==='reduce'?'Reduce one variable and rebuild':'Hold the current difficulty'),
    el('div',{class:'progression-heading'},badge(progression.dimension.replaceAll('-',' '),progression.direction==='advance'?'accent':'neutral'),el('strong',{},progression.summary)),
    el('p',{class:'pre-line'},progression.cue),
    el('p',{class:'field-hint'},'The progression engine changes one challenge dimension at a time. A normal Start practice remains available when you do not want the recommendation.'),
    el('div',{class:'actions wrap'},button('Start next challenge',()=>launchPractice([challengeBlock()]),'primary','play'),button('Add next challenge to today',()=>addToday(challengeBlock()),'secondary','plus'))));
