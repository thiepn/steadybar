...
export function sessionPage(id:string,review=false):Page{
  ...
  for(const [i,block] of session.blocks.entries()){
    ...
    records.append(el('article',{class:'review-block'},el('div',{class:'split'},el('div',{},el('div',{class:'eyebrow'},`BLOCK ${String(i+1).padStart(2,'0')} · ${titleCase(block.categorySnapshot)}`),el('h2',{},block.titleSnapshot)),badge(block.skipped?'Skipped':block.completed?'Completed':'Restarted / ended')),
      ...
      block.setPrepSnapshot?el('div',{class:'set-prep-review'},badge('Set prep',block.prescriptionSnapshot?.intent==='perform'?'accent':'neutral'),el('strong',{},block.setPrepSnapshot.setlistName),el('p',{class:'muted small'},`${block.setPrepSnapshot.stage.replaceAll('-',' ')} · ${block.setPrepSnapshot.role.replaceAll('-',' ')} · set position ${block.setPrepSnapshot.setPosition+1}${block.setPrepSnapshot.performanceDate?` · ${formatDate(block.setPrepSnapshot.performanceDate)}`:''}`)):null,
