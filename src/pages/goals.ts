...
export function goalsPage():Page{
  const data=store.view(),activePlan=(data.trainingPlans??[]).find(plan=>plan.status==='active'),page=el('div',{class:'page'},pageHeader('','Goals','Targets for this profile, repertoire, and weekly practice.',[link('Training Cycles','/cycles','button secondary','routine'),button('New goal',()=>editGoal(),'primary','plus')]));
