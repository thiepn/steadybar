...
export function progressPage():Page{
  ...
  if(!finishedSessions(data.sessions).length){page.append(empty('Your progress starts with a session.','Nothing is estimated or invented here. Practice, record an honest attempt, and return to see what changed.',link('Start practice','/practice','button primary','play'),'progress'));return {node:page};}
  ...
  const draw=()=>{
    ...
    else for(const g of data.goals.slice(0,5)){const p=goalProgress(g,data);goals.append(el('div',{class:'goal-summary'},el('strong',{},g.title),el('p',{class:'small muted'},p.label),progressBar(p.fraction,g.title)));}
