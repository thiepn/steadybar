...
export function progressPage():Page{
  ...
  const data=store.view(),profile=activeProfile(store.snapshot()),page=el('div',{class:'page'},pageHeader('','Progress',`${profile.name} · Practice time, evidence trends and explainable diagnostics.`,[link('Weekly Review','/review','button secondary','progress'),link('Timing Lab','/timing-lab','button secondary','pulse')]));
