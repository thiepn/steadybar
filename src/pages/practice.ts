...
export function activePracticePage():Page{
  ...
  const tools=el('details',{class:'focus-drawer focus-tools'},
    ...
    el('div',{class:'focus-drawer-body focus-tool-grid'},
      recordButton,button('Quick note',note,'ghost','note'),timingButton,activeProfile(store.snapshot()).instrumentType==='drums'?link('MIDI Drum Lab','/midi-lab','button ghost','pulse'):null,trainerButton,restart,skip,finishUnrated,recordingStatus));
  ...
  const identity=el('section',{class:'practice-identity focus-identity'},
    el('div',{class:'focus-meta'},intentBadge,setPrepBadge,progressionBadge),title,sticking,progressionCue);
  ...
  function draw():void{
    ...
    if(lastState!==state){
      ...
      const prescription=block.prescriptionSnapshot,progression=block.progressionSnapshot,setPrep=block.setPrepSnapshot;
      intentBadge.hidden=!prescription;text(intentBadge,prescription?`${prescription.generatedBy==='autopilot'?'Autopilot · ':prescription.generatedBy==='set-prep'?'Set prep · ':''}${prescription.intent.replaceAll('-',' ')}`:'');
      setPrepBadge.hidden=!setPrep;text(setPrepBadge,setPrep?`Set · ${setPrep.stage.replaceAll('-',' ')} · ${setPrep.role.replaceAll('-',' ')}`:'');
