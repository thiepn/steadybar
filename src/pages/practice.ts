...
export function activePracticePage():Page{
  ...
  const onKey=(event:KeyboardEvent)=>{
    ...
    else if(event.key==='ArrowUp'||event.key==='ArrowDown'){event.preventDefault();void practice.setBpm(practice.session!.runtime.bpm+(event.key==='ArrowUp'?1:-1)*(event.shiftKey?5:1)).catch(e=>notify(e.message,'error'));}
