import { DEFAULT_SETTINGS, type Category, type Data, type Exercise, type Routine, type RoutineBlock } from '../domain/models.js';
import { nowISO } from '../domain/utils.js';
const rudiments: [string,string,string][] = [
...
const extras: [string,Category,string,string][] = [
...
export function seedData(timestamp = nowISO()): Data {
  ...
  const exercises: Exercise[] = rudiments.map(([name,sticking,description],i) => ({...base,id:`rudiment-${i+1}`,name,sticking,description,instructions:description,category:'rudiment',tags:['hands','rudiment']}));
  ...
  const ex = (id:string, minutes:number): RoutineBlock => {
  ...
  const free = (title:string, minutes:number): RoutineBlock => ({id:`block-${title.toLowerCase().replaceAll(' ','-')}`,type:'free',title,targetSeconds:minutes*60,bpm:80,notes:title.startsWith('Song') ? 'Replace this free block with a song from your library, or use it for song practice without a saved song.' : '',order:0});
  const definitions: [string,string,RoutineBlock[]][] = [
  ...
  const routines: Routine[] = definitions.map(([name,description,blocks],i) => ({id:`routine-${i+1}`,name,description,blocks:blocks.map((b,order) => ({...b,id:`template-${i}-${order}`,order})),scheduledDays:[],tags:['starter'],builtin:true,archived:false,createdAt:timestamp,updatedAt:timestamp}));
