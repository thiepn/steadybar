...
export const STORES = ['profiles','courseProgress','exercises','songs','routines','dailyPlans','sessions','goals','setlists','trainingPlans','weeklySchedules','recordings','timingResults','practiceStates','priorityCycles','metronomePresets','settings'] as const;
export type StoreName = typeof STORES[number];
export interface StoreTypes { courseProgress:CourseProgress; profiles:PracticeProfile; exercises:Exercise; songs:Song; routines:Routine; dailyPlans:DailyPlan; sessions:PracticeSession; goals:Goal; setlists:Setlist; trainingPlans:TrainingPlan; weeklySchedules:WeeklySchedule; recordings:PracticeRecording; timingResults:TimingLabResult; practiceStates:PracticeState; priorityCycles:PriorityCycle; metronomePresets:Preset; settings:Settings }
const validators: { [K in StoreName]: Validator<StoreTypes[K]> } = {courseProgress:validateCourseProgress,profiles:validateProfile,exercises:validateExercise,songs:validateSong,routines:validateRoutine,dailyPlans:validatePlan,sessions:validateSession,goals:validateGoal,setlists:validateSetlist,trainingPlans:validateTrainingPlan,weeklySchedules:validateWeeklySchedule,recordings:validatePracticeRecording,timingResults:validateTimingLabResult,practiceStates:validatePracticeState,priorityCycles:validatePriorityCycle,metronomePresets:validatePreset,settings:validateSettings};
  ...
  const promise = new Promise<IDBDatabase>((resolve,reject) => {
    ...
    req.onupgradeneeded=event=>{
      try{
        ...
        if(event.oldVersion<6&&!db.objectStoreNames.contains('trainingPlans'))db.createObjectStore('trainingPlans',{keyPath:'id'});
...
async function referenceSnapshot(tx:IDBTransaction):Promise<Data>{
  ...
  if(data.profiles?.length){data.schemaVersion=2;data.practiceModelVersion=1;data.trainingPlans??=[];data.weeklySchedules??=[];data.recordings??=[];data.timingResults??=[];data.practiceStates??=[];data.priorityCycles??=[];}return data;
}
...
export async function readData():Promise<Data> {
  ...
  try{
    ...
    if((source.profiles as unknown[])?.length){source.schemaVersion=2;source.practiceModelVersion=1;source.trainingPlans??=[];source.weeklySchedules??=[];source.recordings??=[];source.timingResults??=[];source.practiceStates??=[];source.priorityCycles??=[];}
    else {if(!(source.courseProgress as unknown[])?.length)delete source.courseProgress;delete source.trainingPlans;delete source.weeklySchedules;delete source.recordings;delete source.timingResults;delete source.practiceStates;delete source.priorityCycles;delete source.practiceModelVersion;}
...
export async function mutateWorkspace(fn:(data:Data)=>Data):Promise<Data>{
  return write([...STORES],async tx=>{
    ...
    current.schemaVersion=2;current.practiceModelVersion=1;current.trainingPlans??=[];current.weeklySchedules??=[];current.recordings??=[];current.timingResults??=[];current.practiceStates??=[];current.priorityCycles??=[];
...
export async function initializeDatabase():Promise<void> {
  await write([...STORES,'migrationBackups'],async tx=>{
    ...
    if(prefs && profileRows?.length){
      const current=Object.fromEntries(STORES.map((name,i)=>[name,name==='settings'?prefs:rows[i]])) as unknown as Data;current.schemaVersion=2;current.trainingPlans??=[];current.weeklySchedules??=[];current.recordings??=[];current.timingResults??=[];
    ...
    const previous=prefs ? Object.fromEntries(STORES.filter(n=>!['profiles','courseProgress','trainingPlans','weeklySchedules','recordings','timingResults','practiceStates','priorityCycles'].includes(n)).map(name=>[name,name==='settings'?prefs:rows[STORES.indexOf(name)]])) as unknown as Data : seedData();
