...
export async function mutateWorkspace(fn:(data:Data)=>Data):Promise<Data>{
  return write([...STORES],async tx=>{
    ...
    current.schemaVersion=2;current.practiceModelVersion=1;current.trainingPlans??=[];current.weeklySchedules??=[];current.recordings??=[];current.timingResults??=[];current.midiDeviceProfiles??=[];current.midiResults??=[];current.midiDeviceProfiles??=[];current.midiResults??=[];current.practiceStates??=[];current.priorityCycles??=[];
