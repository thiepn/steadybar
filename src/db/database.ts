...
import type { Data, DailyPlan, Exercise, Goal, PracticeSession, Preset, Routine, Setlist, Settings, Song, TrainingPlan, WeeklySchedule } from '../domain/models.js';
import type { PracticeState, PriorityCycle } from '../domain/practice-state.js';
import { DEFAULT_SETTINGS } from '../domain/models.js';
import { validateProfile, validateData, validateExercise, validateGoal, validatePlan, validatePreset, validateRoutine, validateSession, validateSetlist, validateSettings, validateSong, validateTrainingPlan, validateWeeklySchedule, type Validator } from '../domain/validation.js';
...
export const STORES = ['profiles','courseProgress','exercises','songs','routines','dailyPlans','sessions','goals','setlists','trainingPlans','weeklySchedules','practiceStates','priorityCycles','metronomePresets','settings'] as const;
export type StoreName = typeof STORES[number];
export interface StoreTypes { courseProgress:CourseProgress; profiles:PracticeProfile; exercises:Exercise; songs:Song; routines:Routine; dailyPlans:DailyPlan; sessions:PracticeSession; goals:Goal; setlists:Setlist; trainingPlans:TrainingPlan; weeklySchedules:WeeklySchedule; practiceStates:PracticeState; priorityCycles:PriorityCycle; metronomePresets:Preset; settings:Settings }
const validators: { [K in StoreName]: Validator<StoreTypes[K]> } = {courseProgress:validateCourseProgress,profiles:validateProfile,exercises:validateExercise,songs:validateSong,routines:validateRoutine,dailyPlans:validatePlan,sessions:validateSession,goals:validateGoal,setlists:validateSetlist,trainingPlans:validateTrainingPlan,weeklySchedules:validateWeeklySchedule,practiceStates:validatePracticeState,priorityCycles:validatePriorityCycle,metronomePresets:validatePreset,settings:validateSettings};
...
async function referenceSnapshot(tx:IDBTransaction):Promise<Data>{
  ...
  if(data.profiles?.length){data.schemaVersion=2;data.practiceModelVersion=1;data.trainingPlans??=[];data.weeklySchedules??=[];data.practiceStates??=[];data.priorityCycles??=[];}return data;
}
...
export async function mutateWorkspace(fn:(data:Data)=>Data):Promise<Data>{
  return write([...STORES],async tx=>{
    ...
    current.schemaVersion=2;current.practiceModelVersion=1;current.trainingPlans??=[];current.weeklySchedules??=[];current.practiceStates??=[];current.priorityCycles??=[];
...
export async function initializeDatabase():Promise<void> {
  await write([...STORES,'migrationBackups'],async tx=>{
    ...
    if(prefs && profileRows?.length){
      const current=Object.fromEntries(STORES.map((name,i)=>[name,name==='settings'?prefs:rows[i]])) as unknown as Data;current.schemaVersion=2;current.trainingPlans??=[];current.weeklySchedules??=[];
    ...
    const previous=prefs ? Object.fromEntries(STORES.filter(n=>!['profiles','courseProgress','trainingPlans','weeklySchedules','practiceStates','priorityCycles'].includes(n)).map(name=>[name,name==='settings'?prefs:rows[STORES.indexOf(name)]])) as unknown as Data : seedData();
