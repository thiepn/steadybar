...
export function profileView(data: Data,id=activeProfile(data).id): Data {
  ...
  return {...data,courseProgress:data.courseProgress?.filter(p=>p.profileId===id),exercises:data.exercises.filter(e=>e.profileId===id),routines:data.routines.filter(r=>r.profileId===id),
    ...
    sessions:data.sessions.filter(s=>s.profileId===id),trainingPlans:data.trainingPlans?.filter(p=>p.profileId===id),weeklySchedules:data.weeklySchedules?.filter(w=>w.profileId===id),recordings:data.recordings?.filter(r=>r.profileId===id),timingResults:data.timingResults?.filter(r=>r.profileId===id),midiDeviceProfiles:data.midiDeviceProfiles?.filter(r=>r.profileId===id),midiResults:data.midiResults?.filter(r=>r.profileId===id),practiceStates:data.practiceStates?.filter(s=>s.profileId===id),priorityCycles:data.priorityCycles?.filter(c=>c.profileId===id)};
}
